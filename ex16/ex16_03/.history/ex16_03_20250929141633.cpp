...
void histogram(const cv::Mat image);
...
// Define parameters for the histogram
    ...
    // Draw the histogram
    int binWidth = cv::cvRound((double) histImage.cols / histSize);
    for (int i = 1; i < histSize; i++) {
        cv::line(histImage,
             cv::Point(binWidth * (i - 1), histImage.rows - cv::cvRound(hist.at<float>(i - 1))),
             cv::Point(binWidth * i, histImage.rows - cv::cvRound(hist.at<float>(i))),
